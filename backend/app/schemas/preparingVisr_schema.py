...
import pandas as pd
from const.pandas_template import LaborEnum, AdditionalEstimatedEnum, VisrDataEnum
...
class PreparingVisr(BaseModel, EstimateInterface):
    ...
    def set_estimated_price(self) -> DataFrame:
        ...
        _temp_dt.loc[cond, 'quantity':'total_cost'] = _temp_dt.loc[cond,
                                                                   'quantity':'total_cost'].astype('Float64')
        _temp_dt.loc[cond, 'temp'] = VisrDataEnum.estimated_price.value
        return _temp_dt
    ...
    def set_additional_estimated(self) -> DataFrame:
        ...
        for additional_type in AdditionalEstimatedEnum:
            ...
            additional_df.loc[:, 'total_cost'] = additional_df.loc[:, 'total_cost'].astype(
                'Float64')
    ...
    def set_total_estimate_cost(self) -> None:
        ...
        try:
            if (('итог' in self.data.iloc[-1]['name'].lower()) & (pd.isna(self.data.iloc[-1]['code']))):
                self.data.loc[self.data.index[-1],
                              'total_cost'] = float(self.data.iloc[-1]['total_cost'])
                self.data.loc[self.data.index[-1], 'temp'] = 'T'
        except:
            raise HTTPException(
                422, detail='Отловить строку итоговой стоимости')
