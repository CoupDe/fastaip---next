...
from sqlalchemy import asc, delete, desc, insert
...
async def read_all_buildings(session: AsyncSession) -> list[Building]:
    ...
    """
    stmt = select(Building).where(Building.id > 0).options(
        selectinload(Building.structure)).order_by(asc(Building.name))
...
async def get_building_by_code_name(name: str, code_building: str, session: AsyncSession):
    building = await session.scalar(select(Building).where(or_(Building.name == name, Building.code_building == code_building)))
    return building


async def create_building(building: building_schema.CreateBuilding, session: AsyncSession) -> Building | None:
    result = await session.scalar(insert(Building).values(**building.dict()
                                                          ).returning(Building))
...
async def delete_building(id: int, session: AsyncSession) -> str | None:
    stmt = delete(Building).where(
        Building.id == id).returning(Building.name)
